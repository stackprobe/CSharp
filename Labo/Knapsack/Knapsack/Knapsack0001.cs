...
namespace Charlotte
{
	public class Knapsack0001 : IKnapsack
	{
		...
		private int ToIndex(int i, int w)
		{
			return i * (this.Cond.Capacity + 1) + w;
		}
		...
		public int GetBestValue()
		{
			...
			for (int i = 0; i < this.Cond.Items.Length; i++)
			{
				...
				if (w <= this.Cond.Capacity)
				{
					...
					ret = Math.Max(ret, v);
					item_weight2Value[this.ToIndex(i, w)] = v;
				}
			}
			for (int i = 0; i < this.Cond.Items.Length; i++)
			{
				...
				{
					int v = item_weight2Value[this.ToIndex(i, w)];
					...
					if (v != 0) // .Value == 0 のアイテムだけの組み合わせは無視して良い。
					{
						for (int ii = i + 1; ii < this.Cond.Items.Length; ii++)
						{
							...
							if (ww <= this.Cond.Capacity)
							{
								...
								if (item_weight2Value[this.ToIndex(ii, ww)] < vv)
								{
									ret = Math.Max(ret, vv);
									item_weight2Value[this.ToIndex(ii, ww)] = vv;
								}
							}
						}
					}
