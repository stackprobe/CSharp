		...
		public void Test01_a(int th_num)
		{
			...
			for (int c = 0; c < th_num; c++)
			{
				ths[c] = new Thread(() =>
				{
					try
					{
						TimeLimitedTempDir wd = new TimeLimitedTempDir("Module2_Test01_a_WorkingDirectory");
			...
			for (int c = 0; c < th_num; c++)
			{
				ths[c] = new Thread(() =>
				{
					try
					{
						for (int d = 0; d < 10; d++)
						{
							TimeLimitedTempDir wd = new TimeLimitedTempDir("Module2_Test01_a_WorkingDirectory", 2);
							string file = wd.MakePath();
